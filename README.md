...
原版是 **Electron 桌面应用**：内嵌浏览器视图浏览平台，进入视频页后自动把"解析接口 + 视频页 URL"
注入为播放器。Docker / Web 版把同样的体验搬到浏览器里：后端提供 **反向代理网关**
（剥掉 `X-Frame-Options` / CSP，注入探测脚本），前端做成 **浏览器风格界面**，
实现 **应用内浏览平台 → 点开视频自动解析 → 浮层播放**，并支持后退/前进/首页、
影视导航模式、服务端持久化的自定义解析接口与导航列表。
...
主要新增：`server/`（Express 后端、`proxy.js` 反向代理、`parsers.js` 解析逻辑、`settings.js` 设置持久化）、
`public/`（浏览器风格前端）、`Dockerfile`、`docker-compose.yml`。详见 [docker/README.md](docker/README.md)。

> 端口可通过 `docker run -p 8080:3000 ...` 自定义；自定义解析接口/导航保存在 Docker 卷 `audiovisual-data` 中。
